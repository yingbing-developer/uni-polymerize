...
export default {
	...
	search (data) {
		...
		Object.keys(api).forEach(key => {
			...
			if ( index == -1 || (index > -1 && sources[index].key > -1) ) {
				if ( !data.isLastPage[key] && (!COMICURL[key].isAdult || adult) && COMICURL[key].search ) {
	...
	getHotKeyword () {
		...
		Object.keys(api).forEach(key => {
			...
			let index = sources.findIndex(source => source.id == key)
			if ( index == -1 || (index > -1 && sources[index].key > -1) ) {
				if ( (!COMICURL[key].isAdult || adult) && COMICURL[key].hotKey ) {
	...
	getRecome () {
		...
		Object.keys(api).forEach(key => {
			...
			let index = sources.findIndex(source => source.id == key)
			if ( index == -1 || (index > -1 && sources[index].key > -1) ) {
				if ( (!COMICURL[key].isAdult || adult) && COMICURL[key].recome ) {
	...
	//获取书籍内容
	getContent (data) {
		return api[data.source].getContent(data)
	},
	//获取评论
	getComment (data) {
