export function getDetail(context, params) {
	return new Promise(resolve => {
		...
		http.get(baseUrl + params.id, {
			...
			let con = str.match(/<div[^>]*class=([""]?)asTBcell uwthumb\1[^>]*>*([\s\S]*?)<\/a>/)[0];
			...
			let author = str.match(/<div[^>]*class=([""]?)asTBcell uwuinfo\1[^>]*>*([\s\S]*?)<\/div>/)[0].match(/<p[^>]*>*([\s\S]*?)<\/p>/)[1];
			...
			let chapters = await http.get(baseUrl + href, {
				...
				for ( let i = 0; i < len; i++ ) {
					...
					data.push(
						new Chapter({
							id: (i + 1) + '_' + source,
							title: '第' + (i + 1) + '部分' + '（共' + content.length + 'P）',
							chapter: i + 1,
							isStart: i == 0,
							isEnd: i == len - 1,
							content: content
